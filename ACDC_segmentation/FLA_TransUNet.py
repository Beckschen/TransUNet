...
class Block(nn.Module):
    def __init__(self, config, vis, attention_type_class):
        ...
        self.attn = attention_type_class(config, vis)  # Use the dynamic attention type
        self.attention_type = str(attention_type_class)
...
class Encoder(nn.Module):
    def __init__(self, config, vis, attention_type_class):
        ...
        for _ in range(config.transformer["num_layers"]):
            ...
            layer = Block(config, vis, attention_type_class)  # Pass the attention type to Block
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, zero_head=False, vis=False):
        ...
        # 从映射字典中获取 attention_type 对应的类

        attention_type_class = attention_type_mapping[config.attention_type]

        # attention_type = config.attention_type  # Get the attention type from config
        self.transformer = Transformer(config, img_size, vis, attention_type_class)  # Pass the attention type to Transformer
